...
FROM zip_fips																			
																						   SELECT npi,
	   drug_name, specialty_description
FROM drug													
...
SELECT npi,																						   drug_name,
	   (SELECT SUM(total_claim_count) AS total_claims FROM prescription)					FROM drug											   
CROSS JOIN prescriber
WHERE specialty_description = 'Pain Management' AND nppes_provider_city = 'NASHVILLE'
AND opioid_drug_flag = 'Y'																						   
    ...
    c. Finally, if you have not done so already, fill in any missing values for total_claim_count with 0. Hint - Google the COALESCE function.
																						   ...
																						   SELECT npi,																						   drug_name,
	   COALESCE ((SELECT SUM(total_claim_count) AS total_claims FROM prescription),0)				FROM drug AS Equal_Zero											   
CROSS JOIN prescriber
WHERE specialty_description = 'Pain Management' AND nppes_provider_city = 'NASHVILLE'
AND opioid_drug_flag = 'Y'			
